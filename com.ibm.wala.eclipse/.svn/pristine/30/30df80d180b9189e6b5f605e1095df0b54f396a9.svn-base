...
public class CGJavaLabelProvider extends LabelProvider {
  ...
  public CGJavaLabelProvider(Map<Integer, IJavaElement> capaNodeIdToJavaElement) {
    ...
    javaEltProvider = new JavaElementLabelProvider(JavaElementLabelProvider.SHOW_PARAMETERS
        | JavaElementLabelProvider.SHOW_OVERLAY_ICONS | JavaElementLabelProvider.SHOW_POST_QUALIFIED
  ...
  public Image getImage(Object element) {
    if (element instanceof Pair) {
      return JavaPlugin.getImageDescriptorRegistry().get(
          new JavaElementImageDescriptor(JavaPluginImages.DESC_MENU_SHIFT_RIGHT, 0, JavaElementImageProvider.SMALL_SIZE));
  ...
  private Image getCapaImage() {
    return JavaPlugin.getImageDescriptorRegistry().get(
        new JavaElementImageDescriptor(JavaPluginImages.DESC_OBJS_CFILE, 0, JavaElementImageProvider.BIG_SIZE));
  }

  private Image getWarningImage() {
    return JavaPlugin.getImageDescriptorRegistry().get(
        new JavaElementImageDescriptor(JavaPluginImages.DESC_OBJS_REFACTORING_WARNING, 0, JavaElementImageProvider.BIG_SIZE));
  }
}
