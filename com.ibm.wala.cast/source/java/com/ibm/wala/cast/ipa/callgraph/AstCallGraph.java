...
public class AstCallGraph extends ExplicitCallGraph {
  ...
  public class AstCGNode extends ExplicitNode {
    ...
    public void updateMethodandClearDU(IMethod m, IR ir) {
      //JEFF
      this.method = m;//why do we need to change the method?
      //if changed, may not be able to find it
      this.updateIRandClearDU(ir);
    }


    public void updateMethod(IMethod m, IR ir) {
      this.method = m;
      this.updateIR(ir);//keep du, since no difference
    }
  }
