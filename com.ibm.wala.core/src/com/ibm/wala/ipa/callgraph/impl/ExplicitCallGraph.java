...
public class ExplicitCallGraph extends BasicCallGraph<SSAContextInterpreter> implements BytecodeConstants {
  ...
  public class ExplicitNode extends NodeImpl {
    ...
    @Override
    public boolean delTarget(CallSiteReference site, CGNode tNode) {
      return delTarget(site.getProgramCounter(),tNode);
    }
    public boolean delTarget(int pc,CGNode tNode) {
      //allTargets.remove(getCallGraph().getNumber(target));
      //      for (IntIterator it = targets.safeIterateIndices(); it.hasNext();) {
      //        int pc = it.next();
    ...
    @Override
    public int hashCode() {
      if(hashCode == 0){//add .toString()
        hashCode = getMethod().toString().hashCode() * 8681 + getContext().hashCode();
      }
      return hashCode;
    }
    ...
    public void updateIRandClearDU(IR new_ir){
      this.ir = new WeakReference<IR>(new_ir);
      this.du.clear();//JEFF -- reset du
    }

    public void updateIR(IR new_ir){
      this.ir = new WeakReference<IR>(new_ir);
    }
