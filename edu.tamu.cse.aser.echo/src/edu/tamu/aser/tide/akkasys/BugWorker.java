...
//			}
	...
	private ArrayList<StartNode> shareGrandParent(int kid1, int kid2) {
		HashMap<Integer, StartNode> mapOfStartNode;
		if(DEBUG){
			mapOfStartNode = Test.engine.mapOfStartNode;
		}else{
		...
		int parentoflater = mapOfStartNode.get(kid2).getParentTID();
		int grandpoflater = mapOfStartNode.get(parentoflater).getParentTID();
		if(grandpoflater == -1){
			return null;
		}else if(kid1 == grandpoflater){
			...
			return shareGrandParent(kid1, parentoflater);
		}
	}
